...
// 等待可预期、不会超时报错。要靠多连接吃并发读的,自己调大 MaxOpenConns,
...
type GTORMSqliteConfig struct {
	DBFilePath      string        `yaml:"dbFilePath" json:"db_file_path"`
...
func (cfg *GTORMSqliteConfig) validate() error {
	...
	if cfg.MaxOpenConns < 0 {
		return fmt.Errorf("gtbox_orm_sqlite: MaxOpenConns must be >= 0, got %d", cfg.MaxOpenConns)
	}
	if cfg.MaxIdleConns < 0 {
		return fmt.Errorf("gtbox_orm_sqlite: MaxIdleConns must be >= 0, got %d", cfg.MaxIdleConns)
	}
	...
	if cfg.MaxOpenConns > 0 && cfg.MaxIdleConns > cfg.MaxOpenConns {
		return fmt.Errorf("gtbox_orm_sqlite: MaxIdleConns(%d) must be <= MaxOpenConns(%d)", cfg.MaxIdleConns, cfg.MaxOpenConns)
	}
	return nil
}
