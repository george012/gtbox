...
func TestGTORMMysqlConfigValidate(t *testing.T) {
	base := GTORMMysqlConfig{DBUser: "root", DBPwd: "pwd", DBName: "shop", DBHost: "127.0.0.1", DBPort: 3306}
	...
	cases := []struct {
		...
		{"空密码合法", func(cfg *GTORMMysqlConfig) { cfg.DBPwd = "" }, false},
		{"缺 User", func(cfg *GTORMMysqlConfig) { cfg.DBUser = "" }, true},
		{"缺 DBName", func(cfg *GTORMMysqlConfig) { cfg.DBName = "" }, true},
		{"缺 Host", func(cfg *GTORMMysqlConfig) { cfg.DBHost = "" }, true},
		{"Port 为 0", func(cfg *GTORMMysqlConfig) { cfg.DBPort = 0 }, true},
		{"Port 越界", func(cfg *GTORMMysqlConfig) { cfg.DBPort = 70000 }, true},
		{"ConnMaxOpen 负数", func(cfg *GTORMMysqlConfig) { cfg.ConnMaxOpen = -1 }, true},
		{"ConnMaxIdle 超过 ConnMaxOpen", func(cfg *GTORMMysqlConfig) { cfg.ConnMaxOpen, cfg.ConnMaxIdle = 4, 8 }, true},
...
func TestGTORMMysqlConfigPoolParams(t *testing.T) {
	...
	if maxOpen != defaultMysqlConnMaxOpen || maxIdle != defaultMysqlConnMaxIdle || idleTime != defaultMysqlConnMaxIdleTime {
		t.Fatalf("零值池参数=%d/%d/%s, want %d/%d/%s", maxOpen, maxIdle, idleTime,
			defaultMysqlConnMaxOpen, defaultMysqlConnMaxIdle, defaultMysqlConnMaxIdleTime)
	}

	customCfg := GTORMMysqlConfig{ConnMaxOpen: 32, ConnMaxIdle: 8, ConnMaxIdleTime: 30 * time.Second}
...
func TestBuildMysqlDSN(t *testing.T) {
	cfg := GTORMMysqlConfig{DBUser: "root", DBPwd: "p@ss:w/ord", DBName: "shop", DBHost: "10.0.0.7", DBPort: 3307}
	...
	if parsed.User != cfg.DBUser || parsed.Passwd != cfg.DBPwd || parsed.DBName != cfg.DBName ||
...
func TestGTORMMysqlOpenWithConfigRejectReopen(t *testing.T) {
	...
	err := aMysql.OpenWithConfig(GTORMMysqlConfig{DBUser: "root", DBName: "shop", DBHost: "127.0.0.1", DBPort: 3306})
